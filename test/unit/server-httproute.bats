...
@test "server/gateway/httproute/tlspolicy: namespace" {
  cd `chart_dir`
  local actual=$(helm template \
      --show-only templates/server-httproute.yaml  \
      --set 'global.tlsDisable=false' \
      ...
      --namespace foo \
      . | tee /dev/stderr |
      yq -r 'select(.kind == "BackendTLSPolicy").metadata.namespace' | tee /dev/stderr)
  [ "${actual}" = "foo" ]
  local actual=$(helm template \
      --show-only templates/server-httproute.yaml  \
      --set 'global.tlsDisable=false' \
...
@test "server/gateway/httproute/tlspolicy: has api version" {
  cd `chart_dir`
  local actual=$(helm template \
      --show-only templates/server-httproute.yaml  \
      --set 'global.tlsDisable=false' \
      --set 'server.gateway.httpRoute.enabled=true' \
      . | tee /dev/stderr |
      yq -r 'select(.kind == "BackendTLSPolicy").apiVersion | length > 0' | tee /dev/stderr)
  [ "${actual}" = "true" ]
}

@test "server/gateway/httproute/tlspolicy: disable by injector.externalVaultAddr" {
  cd `chart_dir`
  local actual=$( (helm template \
      --show-only templates/server-httproute.yaml  \
      --set 'global.tlsDisable=false' \
...
@test "server/gateway/httproute/tlspolicy: labels gets added to object" {
  cd `chart_dir`
  ...
  local actual=$(helm template \
      ...
      --set 'global.tlsDisable=false' \
      --set 'server.gateway.httpRoute.enabled=true' \
      --set 'server.gateway.httpRoute.tlsPolicy.labels.traffic=external' \
      --set 'server.gateway.httpRoute.tlsPolicy.labels.team=dev' \
      . | tee /dev/stderr |
      yq -r 'select(.kind == "BackendTLSPolicy").metadata.labels.traffic' | tee /dev/stderr)
  [ "${actual}" = "external" ]
}

@test "server/gateway/httproute/tlspolicy: annotations added to object - string" {
  cd `chart_dir`
  ...
  local actual=$(helm template \
      ...
      --set 'global.tlsDisable=false' \
      --set 'server.gateway.httpRoute.enabled=true' \
      --set 'server.gateway.httpRoute.tlsPolicy.annotations=kubernetes.io/ingress.class: nginx' \
      . | tee /dev/stderr |
      yq -r 'select(.kind == "BackendTLSPolicy").metadata.annotations["kubernetes.io/ingress.class"]' | tee /dev/stderr)
  [ "${actual}" = "nginx" ]
}

@test "server/gateway/httproute/tlspolicy: annotations added to object - yaml" {
  cd `chart_dir`
...
@test "server/gateway/httproute/tlspolicy: uses active service when ha by default - yaml" {
  cd `chart_dir`
  ...
  local actual=$(helm template \
      ...
      --set 'server.service.enabled=true' \
      . | tee /dev/stderr |
      yq -r 'select(.kind == "BackendTLSPolicy").spec.targetRefs[0].name' | tee /dev/stderr)
  [ "${actual}" = "release-name-openbao-active" ]
}

@test "server/gateway/httproute/tlspolicy: uses regular service when configured with ha - yaml" {
  cd `chart_dir`
...
@test "server/gateway/httproute/tlspolicy: uses regular service when not ha - yaml" {
  cd `chart_dir`
...
@test "server/gateway/httproute/tlspolicy: uses regular service when not ha and activeService is true - yaml" {
  cd `chart_dir`
...
@test "server/gateway/httproute/tlspolicy: validation hostname" {
  cd `chart_dir`
  ...
  local actual=$(helm template \
      ...
      --set 'server.service.enabled=true' \
      . | tee /dev/stderr |
      yq -r 'select(.kind == "BackendTLSPolicy").spec.validation.hostname' | tee /dev/stderr)
  [ "${actual}" = "release-name-openbao" ]
}

@test "server/gateway/httproute/tlspolicy: validation settings" {
  cd `chart_dir`
  ...
  local actual=$(helm template \
      ...
      --set 'server.gateway.httpRoute.activeService=true' \
      --set 'server.service.enabled=true' \
      --set 'server.gateway.httpRoute.tlsPolicy.validation.caCertificateRefs.name=ca-certs' \
      . | tee /dev/stderr |
      yq -r 'select(.kind == "BackendTLSPolicy").spec.validation.caCertificateRefs.name' | tee /dev/stderr)
  [ "${actual}" = "ca-certs" ]
}
